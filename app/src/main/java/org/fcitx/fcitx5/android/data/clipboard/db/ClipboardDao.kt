...
interface ClipboardDao {
    ...
    @Query("SELECT * FROM ${ClipboardEntry.TABLE_NAME} WHERE deleted=0")
    ...
    @Query("SELECT * FROM ${ClipboardEntry.TABLE_NAME} WHERE text LIKE '%' || :query || '%' AND deleted=0 ORDER BY pinned DESC, timestamp DESC")
